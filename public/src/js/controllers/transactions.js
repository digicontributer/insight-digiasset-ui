...
function($scope, $rootScope, $routeParams, $http, $location, Global, Transaction, TransactionsByBlock, TransactionsByAddress, assetTransactionsByAddress, getBlockWithAssetTransactions, TransactionsAsset) {
  ...
  var _aggregateItems = function(items, txid) {
    ...
    for(var i=0; i < l; i++) {
      ...
      if(items[i].assets && items[i].assets.length) {
        ...
        tmp[addr].assetIcon = items[i].assetMeta.assetIcon;
  ...
  var _processAssetTx = function(tx) {
    
  };

  var _processTX = function(tx) {
    tx.vinSimple = _aggregateItems(tx.vin, tx.txid);
    tx.voutSimple = _aggregateItems(tx.vout, tx.txid);
  };
